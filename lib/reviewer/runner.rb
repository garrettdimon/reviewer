...
module Reviewer
  ...
  class Runner
    ...
    def run
      ...
      identify_tool if streaming_output?

      # Use the provided strategy to run the command
      execute_strategy

      # If it failed, display guidance to help them get back on track (only in streaming mode)
      guidance.show unless success? || !streaming_output?

      # Return the exit status generated by the tool as interpreted by the Result
      exit_status
    end

    # Whether output should be streamed (not captured for later formatting)
    #
