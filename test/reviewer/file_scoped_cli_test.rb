...
module Reviewer
  class FileScopedCLITest < Minitest::Test
    ...
    def test_cli_preserves_file_scope_across_every_selector # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
      Dir.mktmpdir do |directory|
        ...
        assert_run(directory, %w[staged],
                   expected: [invocation('file_aware', 'both.rb', 'staged_only.rb')], skipped: 1)
        assert_run(directory, %w[unstaged],
                   expected: [invocation('file_aware', 'both.rb', 'unstaged_only.rb')], skipped: 1)
        assert_run(directory, %w[modified],
                   expected: [invocation('file_aware', 'both.rb', 'staged_only.rb', 'unstaged_only.rb')], skipped: 1)
        assert_run(directory, %w[untracked],
                   expected: [invocation('file_aware', 'untracked.rb')], skipped: 1)
        assert_run(directory, %w[staged untracked],
                   expected: [invocation('file_aware', 'both.rb', 'staged_only.rb', 'untracked.rb')], skipped: 1)
    ...
    def prepare_project(directory) # rubocop:disable Metrics/AbcSize
